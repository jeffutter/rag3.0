...
export function createRAGQueryPipeline(
  ...
  >(
    ...
    async ({ input, context }) => {
      ...
      const systemPrompt = input.systemPrompt || `You are a helpful assistant with access to a knowledge base. Use the search_knowledge_base tool to find relevant information when needed.`;
...
export function createRAGQueryRegistration(
  ...
  return {
    name: 'rag_query',
    description: 'Answer questions using RAG (Retrieval Augmented Generation). Searches the knowledge base for relevant information and generates a comprehensive answer.',
