 ...
 */
export function createBatchProcessingPipeline(contextBuilder: () => EmbeddingContext) {
  return (
    Pipeline.start<Document[], EmbeddingContext>(contextBuilder)
      ...
      .map(
        ...
        >("processBatch", async ({ input, context }) => {
          ...
          for (const doc of input) {
            ...
            const embedding = await generateMockEmbedding(doc.content, context.model);
      ...
      .add(
        ...
        >("generateReport", async ({ input, state, context: _context }) => {
          const totalDocuments = state.batches.reduce((sum, batch) => sum + batch.length, 0) || 0;
          ...
          const totalProcessingTimeMs = batchMetrics.reduce((sum, m) => sum + m.processingTimeMs, 0);

          const avgEmbeddingDimensions =
            input.length > 0 ? input.reduce((sum, e) => sum + e.dimensions, 0) / input.length : 0;
 ...
 */
async function generateMockEmbedding(text: string, model: string): Promise<number[]> {
  // Simulate API delay
...
export async function runBatchProcessingExample() {
  ...
  if (result.success) {
    ...
    console.log("  Total Processing Time:", report.totalProcessingTimeMs.toFixed(2), "ms");
    console.log("  Avg Embedding Dimensions:", report.avgEmbeddingDimensions.toFixed(0));
...
export type { Document, DocumentEmbedding, BatchMetrics, ProcessingReport, EmbeddingContext };
