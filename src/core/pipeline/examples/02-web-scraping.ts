 ...
 */
export function createWebScrapingPipeline(contextBuilder: () => ScrapingContext) {
  return (
    Pipeline.start<WebPage[], ScrapingContext>(contextBuilder)
      ...
      .flatMap(
        "extractedLinks",
        createStep<PageContent, ExtractedLink[], { fetchedPages: PageContent[] }, ScrapingContext>(
          "extractLinks",
          async ({ input, context }) => {
      ...
      .add(
        ...
        >("generateStats", async ({ input, state }) => {
          ...
          return {
            ...
            successfulFetches: state.fetchedPages.filter((p) => p.statusCode === 200).length,
...
export type { WebPage, PageContent, ExtractedLink, ScrapingStats, ScrapingContext };
