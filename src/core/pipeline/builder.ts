...
import type { AddToState, Step, StepError, StepExecutionContext, StepMetadata, StepResult } from "./types";
...
> {
  ...
  add<TKey extends string, TNextOutput>(
    ...
    ? never
    : Pipeline<TInitialInput, TNextOutput, AddToState<TAccumulatedState, TKey, TNextOutput>, TContext> {
    return new Pipeline(
  ...
  branch<TKey extends string, TBranchOutput>(
    ...
    ? never
    : Pipeline<TInitialInput, TBranchOutput, AddToState<TAccumulatedState, TKey, TBranchOutput>, TContext> {
  ...
  map<TKey extends string, TOutput>(
    key: TKey,
    step: TCurrentOutput extends (infer TElement)[] ? Step<TElement, TOutput, TAccumulatedState, TContext> : never,
    options?: SingleToListOptions,
  ): TCurrentOutput extends unknown[]
    ? TKey extends keyof TAccumulatedState
      ? never
      : Pipeline<TInitialInput, TOutput[], AddToState<TAccumulatedState, TKey, TOutput[]>, TContext>
    : never {
  ...
  flatMap<TKey extends string, TOutput>(
    ...
    const flatMappedStep: Step<TCurrentOutput, TOutput[], TAccumulatedState, TContext> = {
  ...
  batch<TKey extends string>(
    ...
    ? TKey extends keyof TAccumulatedState
      ? never
      : Pipeline<TInitialInput, TElement[][], AddToState<TAccumulatedState, TKey, TElement[][]>, TContext>
    : never {
    const batchStep = createBatchStep<ArrayElement<TCurrentOutput>, TAccumulatedState, TContext>(size, `batch_${size}`);
  ...
  flatten<TKey extends string>(
    ...
    ? TKey extends keyof TAccumulatedState
      ...
      : Pipeline<TInitialInput, TElement[], AddToState<TAccumulatedState, TKey, TElement[]>, TContext>
    : never {
    const flattenStep = createFlattenStep<ArrayElement<ArrayElement<TCurrentOutput>>, TAccumulatedState, TContext>(
      "flatten",
    );
  ...
  filter<TKey extends string>(
    ...
    ? TKey extends keyof TAccumulatedState
      ...
      : Pipeline<TInitialInput, TElement[], AddToState<TAccumulatedState, TKey, TElement[]>, TContext>
    : never {
    const filterStep = createFilterStep<ArrayElement<TCurrentOutput>, TAccumulatedState, TContext>(
      predicate,
      `filter_${key}`,
    );
  ...
  async execute(input: TInitialInput): Promise<StepResult<TCurrentOutput>> {
    ...
    for (const stage of this.stages) {
      ...
      try {
        const result = await this.executeWithRetry(stage.step, currentData, accumulatedState, context, traceId, spanId);
  ...
  private async executeWithRetry<I, O, S, C>(
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      const shouldRetry =
        ...
        attempt < maxAttempts &&
        (!step.retry?.retryableErrors || step.retry.retryableErrors.includes(lastResult.error.code));
