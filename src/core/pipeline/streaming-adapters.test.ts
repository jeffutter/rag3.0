...
describe("Streaming Adapters", () => {
	describe("toStreamingStep", () => {
		...
		test("preserves retry configuration", async () => {
			const stepWithRetry = createStep<string, string>(
				"withRetry",
	...
	describe("Step Categorization", () => {
		...
		test("categorizes aggregation steps", () => {
			...
			const groupStep = createStep<string[], Record<string, string[]>>("groupBy", async ({ input }) => ({}));
		...
		test("categorizes reduction steps", () => {
			const filterStep = createStep<number[], number[]>("filter", async ({ input }) =>
				input.filter((n) => n > 0),
			);
	...
	describe("Round-trip Conversions", () => {
		...
		test("streaming -> batch -> streaming preserves behavior", async () => {
			...
			const batchStep = toBatchStep(originalStep);
			const streamingStep = toStreamingStep(
				createStep<number, number>("triple", async ({ input }) => input * 3),
			);
	...
	describe("Behavioral Equivalence", () => {
		test("toStreamingStep produces same results as original batch step", async () => {
			const batchStep = createStep<string, { original: string; upper: string }>(
				"transform",
				async ({ input }) => ({
					original: input,
