...
export class PipelineRegistry {
   ...
   */
  register<TInput, TOutput, TContext>(pipeline: RegisteredPipeline<TInput, TOutput, TContext>): this {
  ...
  getByTags(tags: string[]): RegisteredPipeline[] {
    return this.getAll().filter((pipeline) => pipeline.tags?.some((tag) => tags.includes(tag)));
  }
