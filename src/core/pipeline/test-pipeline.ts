...
const test1Pipeline = Pipeline.start<string>()
  .add('uppercase', createStep<string, string>('uppercase', async ({ input }) => {
...
const test2Pipeline = Pipeline.start<string>()
  .add('process', createStep<string, Step1Output>('process', async ({ input }) => {
  ...
  .add('analyze', createStep<Step1Output, Step2Output, { process: Step1Output }>(
    'analyze',
    async ({ input, state }) => {
  ...
  .add('summary', createStep<
    Step2Output,
    string,
    { process: Step1Output; analyze: Step2Output }
...
const test3Pipeline = Pipeline.start<number>()
  .add('double', createStep<number, number>('double', async ({ input }) => {
    const result = input * 2;
    console.log(`  Double: ${input} -> ${result}`);
    return result;
  }))
  .add('square', createStep<number, number, { double: number }>('square', async ({ input, state }) => {
    const result = input * input;
    console.log(`  Square: ${input} -> ${result}`);
    console.log(`  (Previous double result was: ${state.double})`);
    return result;
  }))
  .add('add_original', createStep<number, number, { double: number; square: number }>(
    'add_original',
    async ({ input, state }) => {
  ...
  .add('final', createStep<number, string, { double: number; square: number; add_original: number }>(
    'final',
    async ({ input, state }) => {
...
let attemptCount = 0;
const test5Pipeline = Pipeline.start<string>()
  .add('retry_test', createStep<string, string>(
    'retry_test',
    async ({ input }) => {
      attemptCount++;
