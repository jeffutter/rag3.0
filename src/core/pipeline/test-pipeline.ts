...
const test1Pipeline = Pipeline.start<string>()
	...
	.add(
		"length",
		createStep<string, number, { uppercase: string }>(
			"length",
			async ({ input }) => {
				console.log(`  Input: "${input}"`);
...
const test2Pipeline = Pipeline.start<string>()
	...
	.add(
		"analyze",
		createStep<Step1Output, Step2Output, { process: Step1Output }>(
			"analyze",
			async ({ input, state }) => {
				console.log(`  Step 2 - Input:`, input);
				console.log(`  Step 2 - State.process:`, state.process);
				console.log(
					`  Step 2 - Verifying state matches input: ${input === state.process}`,
				);
...
const test3Pipeline = Pipeline.start<number>()
	...
	.add(
		"square",
		createStep<number, number, { double: number }>(
			"square",
			async ({ input, state }) => {
	...
	.add(
		"add_original",
		createStep<number, number, { double: number; square: number }>(
			"add_original",
			async ({ input, state }) => {
				...
				console.log(
					`  (All states - double: ${state.double}, square: ${state.square})`,
				);
				return result;
			},
		),
	...
	.add(
		...
		createStep<
			number,
			string,
			{ double: number; square: number; add_original: number }
		>("final", async ({ input, state }) => {
...
const test4Pipeline = Pipeline.start<string>()
	...
	.add(
		"step2",
		createStep<number, number, { step1: number }>(
			"step2",
			async ({ input: _input }) => {
	...
	.add(
		"step3",
		createStep<number, string, { step1: number; step2: number }>(
			"step3",
			async ({ input }) => {
