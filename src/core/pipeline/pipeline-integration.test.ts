...
describe("Pipeline Integration Tests", () => {
	describe("End-to-end workflows", () => {
		test("complex multi-stage pipeline with state accumulation", async () => {
			...
			const parseStep = createStep<Document, ParsedDoc, unknown, unknown>(
				"parse",
				async ({ input }) => {
			...
			>("extractEntities", async ({ input }) => {
				...
				const names =
					input.sentences.join(" ").match(/[A-Z][a-z]+(?:\s+[A-Z][a-z]+)*/g) ||
					[];
				return {
					names: [...new Set(names)],
					places: [],
			...
			const sentimentStep = createStep<
				Entities,
				Sentiment,
				{ parsed: ParsedDoc; entities: Entities },
				unknown
			...
			if (result.success) {
				...
				expect(result.data.stats.sentiment).toMatch(
			...
			const pipeline = Pipeline.start<Doc[]>()
				...
				.map(
					...
					createStep("enrich", async ({ input }: { input: ProcessedDoc }) => {
						const avgWordLength =
							input.words.reduce((sum, w) => sum + w.length, 0) /
							input.words.length;
						return {
							...input,
				...
				.add(
					...
					createStep("summary", async ({ input }) => ({
						...
						avgWords:
							input.reduce((sum, d) => sum + d.wordCount, 0) / input.length,
					})),
				);
	...
	describe("Edge cases", () => {
		test("handles empty array input", async () => {
			const pipeline = Pipeline.start<string[]>()
				.map(
					"uppercased",
					createStep("upper", async ({ input }: { input: string }) =>
						input.toUpperCase(),
					),
				)
		...
		test("handles array with null/undefined elements gracefully", async () => {
			const pipeline = Pipeline.start<string[]>()
				...
				.map(
					"trimmed",
					createStep("trim", async ({ input }: { input: string }) =>
						input.trim(),
					),
				)
		...
		test("handles deeply nested arrays", async () => {
			const pipeline = Pipeline.start<number[][][]>()
				...
				.add(
					"sum",
					createStep("sum", async ({ input }) =>
						input.reduce((a: number, b: number) => a + b, 0),
					),
				);
		...
		test("propagates errors through complex chains with FAIL_FAST", async () => {
			const pipeline = Pipeline.start<number[]>()
				...
				.map(
					...
					createStep(
						"step3",
						async ({ input }: { input: number }) => input * 3,
					),
					{ errorStrategy: ListErrorStrategy.FAIL_FAST },
				);
	...
	describe("Real-world patterns", () => {
		...
		test("validation and filtering pattern", async () => {
			...
			const validate = createStep<Input, ValidatedInput>(
				"validate",
				async ({ input }) => {
					const errors: string[] = [];
