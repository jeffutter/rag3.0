...
describe("createListStep", () => {
	test("creates a list step with custom execute function", async () => {
		const sortStep = createListStep<string, string>(
			"sort",
			async ({ input }) => {
				return [...input].sort();
			},
	...
	test("provides access to state and context", async () => {
		const step = createListStep<string, string, { count: number }>(
			"withState",
			async ({ input, state }) => {
...
describe("createBatchStep", () => {
	...
	test("throws error for invalid batch size", () => {
		expect(() => createBatchStep(0)).toThrow(
			"Batch size must be greater than 0",
		);
		expect(() => createBatchStep(-5)).toThrow(
			"Batch size must be greater than 0",
		);
	});
...
describe("createFilterStep", () => {
	test("filters array based on predicate", async () => {
		const filterEven = createFilterStep<number>(
			(n) => n % 2 === 0,
			"filterEven",
		);
	...
	test("provides index to predicate", async () => {
		const filterByIndex = createFilterStep<string>(
			(_, index) => index % 2 === 0,
			"filterByIndex",
		);
...
describe("integration tests", () => {
	...
	test("can use singleToList with other list operations", async () => {
		...
		const uppercaseList = singleToList(uppercase);
		const filterLong = createFilterStep<string>(
			(s) => s.length > 3,
			"filterLong",
		);
