...
describe("Pipeline.map()", () => {
  test("maps a step over an array with type safety", async () => {
    const pipeline = Pipeline.start<string[]>()
      ...
      .map(
        "uppercased",
        createStep("uppercase", async ({ input }: { input: string }) => input.toUpperCase()),
      );
  ...
  test("accumulates state correctly", async () => {
    const pipeline = Pipeline.start<string[]>()
      ...
      .map(
        "lengths",
        createStep("length", async ({ input }: { input: string }) => input.length),
      )
      .add(
        "total",
...
describe("Pipeline.flatMap()", () => {
  test("flatMaps a step that returns arrays", async () => {
    const pipeline = Pipeline.start<string[]>()
      ...
      .flatMap(
        "words",
        createStep("split", async ({ input }: { input: string }) => input.split(" ")),
      );
  ...
  test("accumulates state correctly", async () => {
    const pipeline = Pipeline.start<string[]>()
      ...
      .flatMap(
        "chars",
        createStep("chars", async ({ input }: { input: string }) => input.split("")),
      )
      .add(
        "count",
...
describe("Complex pipeline chains", () => {
  test("chains multiple list operations together", async () => {
    ...
    const result = await pipeline.execute(["hi", "hello", "world", "a", "test"]);
  ...
  test("combines map and flatMap", async () => {
    const pipeline = Pipeline.start<string[]>()
      ...
      .flatMap(
        "duplicated",
        createStep("duplicate", async ({ input }: { input: string }) => [input, input]),
      )
      .map(
        "uppercased",
        createStep("uppercase", async ({ input }: { input: string }) => input.toUpperCase()),
