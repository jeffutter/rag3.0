...
const simplePipeline = Pipeline.start<string>()
	...
	.add(
		"profile",
		createStep<User, UserProfile, { user: User }>(
			"enrichProfile",
...
const ragPipeline = Pipeline.start<string>()
	...
	.add(
		"search",
		createStep<Embedding, SearchResult[], { embed: Embedding }>(
			"search",
			async ({ input, state }) => {
...
const complexPipeline = Pipeline.start<string>()
	...
	.add(
		...
		>("rerank", async ({ input, state: _state }) => {
			return input.sort((a, b) => b.score - a.score);
		}),
	)
	.add(
...
export async function runExamples() {
	...
	const result3 = await complexPipeline.execute(
