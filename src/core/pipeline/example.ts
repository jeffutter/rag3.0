...
const simplePipeline = Pipeline.start<string>()
  .add(
    "user",
    createStep<string, User>("fetchUser", async ({ input }) => {
      return { id: input, name: "John Doe" };
    }),
  )
  .add(
    "profile",
    createStep<User, UserProfile, { user: User }>("enrichProfile", async ({ input, state: _state }) => {
...
const ragPipeline = Pipeline.start<string>()
  .add(
    "embed",
    createStep<string, Embedding>("embed", async ({ input }) => {
      return {
        vector: [0.1, 0.2, 0.3],
        text: input,
      };
  ...
  .add(
    "search",
    createStep<Embedding, SearchResult[], { embed: Embedding }>("search", async ({ input, state }) => {
...
const complexPipeline = Pipeline.start<string>()
  ...
  .add(
    "search",
    createStep<Embedding, SearchResult[], { embed: Embedding }>("search", async ({ input: _input, state }) => {
  ...
  .add(
    "rerank",
    createStep<SearchResult[], SearchResult[], { embed: Embedding; search: SearchResult[] }>(
      "rerank",
  ...
  .add(
    ...
    >("analyze", async ({ input, state }) => {
      ...
      if (topResult && topResult.score > 0.85) {
        return {
          decision: "good",
          reason: `High confidence match for "${originalQuery}"`,
        };
      }
...
export { simplePipeline, ragPipeline, complexPipeline };
