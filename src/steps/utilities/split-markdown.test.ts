...
describe("isValidChunk", () => {
	...
	test("accepts valid content", () => {
		...
		expect(
			isValidChunk("A paragraph with multiple sentences. Here is another."),
		).toBe(true);
...
describe("getChunkUUID", () => {
	...
	test("throws error for negative index", () => {
		expect(() => getChunkUUID("test-source", -1)).toThrow(
			"Chunk index must be between 0 and 255",
		);
	});

	test("throws error for index > 255", () => {
		expect(() => getChunkUUID("test-source", 256)).toThrow(
			"Chunk index must be between 0 and 255",
		);
	});
...
This is content for section ${i + 1}. It has enough text to be its own chunk.
	...
	test("validates input schema", () => {
		// Valid input
		expect(() =>
			SplitMarkdownInputSchema.parse({ content: "test" }),
		).not.toThrow();
