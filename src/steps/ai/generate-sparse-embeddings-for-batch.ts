...
import { generateSparseEmbeddings, type SparseEmbedding } from "../../lib/sparse-embeddings";
 ...
 * ```
 */
export function createGenerateSparseEmbeddingsForBatchStep(config: SparseEmbeddingConfig = {}) {
  return createStep<
    ChunkData[],
  ...
  >("generateSparseEmbeddings", async ({ input }) => {
    ...
    try {
      ...
      const embeddings = await generateSparseEmbeddings(contents, config.endpoint);
